...
- 当前工作台首屏必须优先展示业务场景卡片,而不是先让用户从模型名开始选择。
- 业务场景卡片应覆盖至少以下默认入口:
  - `工艺单快速整理`
  - `批注翻译给版房`
  - `客户回复草稿`
- 高级设置默认折叠,模型与工具链仅在需要时展开。
- 本地 `Gemma 4 31B Local` 必须可作为高频场景的默认联调模型入口;线上模型仅作为可切换补充。
- UI 验证脚本若失败,需区分:
  - 前端交互/选择器问题
  - 业务场景配置问题
  - 本地模型服务不可达
...
- PDF 输出必须保证业务可确认性:
  ...
  - 对非 dense 页，正式 annotated 应默认优先尝试"离原英文最近的空白处"页内中文说明，不得再把 marker + 右侧栏当作默认主路径
  - 页内近场中文应按距离与留白优先级选位：优先同侧近邻空白，再尝试上下近邻空白；只有局部放不下时才允许该条回退到侧栏
  - 单页 note 数量即使偏多，只要该页所有业务 note 仍可完整落入同页近场空白区，就不得仅因计数阈值机械切到 dense marker/review 模式
  - 稀疏页不应默认退化成整页右侧说明栏
  - **禁止生成额外 CN Review 页**：翻译内容必须放在同一页的右侧 CN Notes 栏内，绝不允许因内容过多而溢出到下一页生成独立的 "CN Review" 页面；右侧栏必须通过自动增加列数（最多 4 列）和缩小字号来适配任意数量的翻译条目，保证人在同一页就能对照英文原文阅读中文翻译
  - 右侧 CN Notes 栏宽度不得低于 340pt，确保多列排版时每列有足够可读空间
  - 密集页（条目多、字符多）必须统一走右侧 panel 模式，通过渐进式压缩（2 列 → 3 列 → 4 列）将所有翻译内容容纳在同一页内；不得因内容密集而回退到原文下方的 dense 内联模式或生成补充 review 页
  ...
  - 当单页 business note 数量已明显拥挤时，正式 annotated 应自动切到右侧 panel 多列紧凑模式：同一业务框里的连续 OCR 行应尽量合并成区间标号，通过右侧 CN Notes 栏多列排版在同一页内承接全部详情；不得继续把 `7,8,9,10,11...` 这样的长串小号直接堆在原页上，也不得生成独立的 CN Review 额外页面
  ...
  - 页面化产品层必须直接给出结构化人工复核建议,不得要求业务员自己去读 comparison 或 JSON 才知道要看哪里;至少要明确:
    - 先看哪几页
