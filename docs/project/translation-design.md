...
## 本机 OpenAI 兼容端与 ModelScope 并存（2026-04-05）

- `qwen-client.ts` 支持 **`LLM_PREFER_OPENAI_COMPAT=1`**：优先使用 `OPENAI_BASE_URL` + `OPENAI_API_KEY`，模型 id 优先 `QWEN_MODEL`（或 `OPENAI_MODEL`），无需清空 `.env.local` 里的 `MODELSCOPE_*`。
- 冒烟：`npm run translate:smoke -- data/test02/M422123.pdf --max-segments 40`（脚本会加载 `.env.local`，密钥仍建议只放本机）。

## 本地多模态页图尺寸（2026-04-04，代码：`vision-render-config.ts` / `pdf-page-raster.ts` / `vision-extraction.ts`）

- **默认（线上/未显式声明本地）**：`VISION_MAX_RENDER_SIZE` 控制 `pdftoppm -scale-to` 的长边像素，默认 **2048**。不改变既有纯文本 A 辅助路径。
- **本地 runtime 单独降分辨率（显式开关）**：设置 `LOCAL_MULTIMODAL_RUNTIME=1` 或 `VISION_USE_LOCAL_RENDER_SIZE=1` 时，改用 `VISION_LOCAL_MAX_RENDER_SIZE`，默认 **1024**，用于减轻本地 `llama.cpp`/gemma 等多模态推理时延与显存压力。
- **多模态 A 辅助（可选）**：仅当 `VISION_MULTIMODAL_ENABLED=1` 时，对低置信块涉及的 PDF 页栅格化为 PNG，并以 OpenAI-compatible `image_url`（data URL）随请求发送；失败时自动回退到原纯文本请求。未设置该变量时，行为与改造前一致（无页图、无 `pdftoppm` 调用）。
- **页数上限**：`VISION_MULTIMODAL_MAX_PAGES`（默认 2，最大 6）限制附带的页图数量。
- **超时**：多模态请求使用 `VISION_MODEL_API_TIMEOUT_MS`（未设则回落到 `MODEL_API_TIMEOUT_MS`），见 `callQwenChatWithContentParts`（`qwen-client.ts`）。
