...
- 因此 mixed 样本的主收敛方向应是：
  ...
  - 再补结构批注短句模板
  - 最后才考虑是否需要换模型
10. 正式 PDF 近场渲染模式（2026-04-09）
- 新一轮业务反馈更强调“工厂人员直接对照使用”，因此正式 annotated 的默认主路径已从“marker + 右侧说明栏”调整为“原英文附近最近空白区中文说明”。
- 当前默认策略是：
  - 非 dense 页：先搜索离 `bbox` 最近的同页空白处，优先右/左近邻，再尝试上下近邻；只有该条确实放不下时才回退到侧栏或 review 页
  - dense 页：仍允许 marker + review 页兜底，但不再把非 dense 页过早降级成整页说明栏
  - 若某页虽然 note 数偏多，但页内近场布局能完整容纳全部业务 note，则应优先直接使用近场布局，不得只因数量阈值提前打回 dense review
  - 近场中文必须严格避开原英文 `bbox` 与已放置中文框；“贴近原文”不等于覆盖原文
  - `FITTING / VOLUME`、`SIZE ... BASE ...` 等尺码框默认不进入正式翻译标注
- 2026-04-09 用当前 business-review 快照复核：
  - `M415013`：Page 1/2 共 `24` 条业务 note，页内近场放置 `24/24`，无 overflow
  - `M445033`：4 页共 `33` 条业务 note，页内近场放置 `33/33`，无 overflow
- 这轮变更的直接目标是压掉“编号很多但要来回跳读”的阅读路径，让中文优先停留在原英文附近的可用空白区。
...
前端工作台已实际切换到“业务场景优先”的入口方式，后续修改默认以此为准：

1. 高频业务场景优先
- 首屏先展示业务场景卡片，再决定角色、模板、工具链与模型
- 当前默认场景包括：
  - `工艺单快速整理`
  - `批注翻译给版房`
  - `客户回复草稿`
...
2. 高级设置默认折叠
- 角色、模板、工具链、识别模型（A）、翻译模型（B）都收进高级设置
- 业务场景点击后应优先自动带入默认模板和工具配置

3. 本地模型默认优先
- 本地 `Gemma 4 31B Local` 作为常规联调优先项
- 线上模型仅作为可切换补充，不再作为高频业务验证的唯一入口

4. UI 验证以业务可用路径为准
- `verify:test02:ui` 需要优先覆盖：
  - 业务场景卡片选择
  - 模型按钮切换
  - 文件上传
  - 提交与 PDF 下载链路
- 若本地模型服务不可达，测试应失败在模型连接层，不应误判为前端交互失效
...
1. 非密集页
- 典型特征：短句批注较少、空白较多
- 目标输出：优先原文附近最近空白区中文说明；仅在局部放不下时回退到编号 + 侧栏

2. 密集页
- 典型特征：表格密集、字段多、说明短而碎
- 目标输出：原页优先保留近场定位与同页空白区说明；局部放不下的剩余内容进入补充审阅页
...
## 页面化与 HITL 结构化（2026-04-10）

这轮开始把 PDF 翻译能力从“可跑通的技术链路”往“业务可直接使用的产品壳层”收。

### 已落地内容

1. 工作台右侧结果区现在新增了结构化“人工复核建议”卡片。
2. 该卡片不读 comparison，也不依赖额外评测文件，而是直接从同一份 `runPdfTranslationPipeline()` 主链结果生成。
3. 页面给出的信息是业务导向的，而不是模型导向的：
   - 先看哪几页
   - 哪些细项最值得人工确认
   - 为什么这些点有业务风险
...
当前由 `feedback-translation.ts` 基于 `pipelineResult.segments + diagnostics` 生成 `metadata.humanReviewGuide`，主要覆盖：
...
这是第一版业务化输出，目标不是“绝对判断对错”，而是明确告诉业务员“先人工看哪里最值”。
...
## Skill 协议最小闭环（2026-04-10）

这轮继续往前收了一层：除了页面可见的 `humanReviewGuide`，当前 PDF 主链还会额外产出稳定的 `pdf_translation_skill_v1` 结果包。
