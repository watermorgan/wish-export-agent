...
## 本地 gemma4 页图与阶段版对比（2026-04-04）

- 仓库已支持：**显式** `LOCAL_MULTIMODAL_RUNTIME` / `VISION_USE_LOCAL_RENDER_SIZE` 下用更小 `VISION_LOCAL_MAX_RENDER_SIZE` 做 `pdftoppm -scale-to`，避免默认影响线上；可选 `VISION_MULTIMODAL_ENABLED` 才向 A 附带页图。详见 `docs/project/translation-design.md` 章节「本地多模态页图尺寸」与 `docs/project/local-gemma4-vs-stage-pdf-comparison.md`。
- 业务结论口径不变：本地 gemma4 **尚不能**在已验证范围内替代阶段版四样本；**M422123** 可作为单样本对照，**M441083 / M445033 / M415013** 需在降页图分辨率后重验证有界时延完成度。
