...
2. 单场景翻译工作台 UX 收口 (P0)
- 状态：进行中
- 说明：当前页面暴露过多系统概念（角色 / 模板 / 技能 / 链路），上传后的下一步不明确，翻译结果入口不够突出；已确认按“默认单场景翻译 + 高级设置折叠 + 结果优先展示 + 页面内打开/下载结果”方向收口。
...
2. 翻译工作台单场景化改造
- 说明：默认首页只服务“意见翻译”，首屏仅保留上传文件、处理要求、开始翻译；角色 / 模板 / 技能收进“高级设置”折叠区。

3. 上传后操作引导与进度提示
- 说明：上传后明确提示默认执行内容、下一步按钮和阶段状态，不再只显示泛化的“处理中...”。

4. 翻译结果优先展示与结果入口
- 说明：结果区顶部固定“翻译结果”模块，优先展示页面内结果，并增加“页面查看 / 打开翻译结果 / 下载翻译 PDF”入口。
...
1. **翻译工作台收口**：先把首页改成“上传文件 + 处理要求 + 开始翻译”的单场景流程，隐藏角色 / 模板 / 技能的默认暴露。
2. **结果入口前置**：把“翻译结果 / 页面查看 / 下载 PDF”固定到结果区顶部，减少业务找结果的成本。
3. **再做页面扩展**：在主翻译链路稳定后，再补独立审核队列和模板管理页面。
4. **Prompt 接入**：将 `skills/*/prompt.md` 接入真实执行链（`execution.ts`）。
